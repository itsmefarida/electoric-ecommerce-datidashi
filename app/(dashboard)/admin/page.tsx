...
export default function VouchersPage() {
  ...
  const getVoucherStatus = (voucher: Voucher) => {
    ...
    // Jika status server adalah Inactive, tampilkan Inactive
    if (!voucher.isActive) {
      return { label: 'Inactive', colorClass: 'bg-red-100 text-red-800' };
    }

    // Cek apakah sudah kadaluarsa (jika expiryDate ada)
  ...
  const handleSelectAll = () => {
    if (selectedVouchers.size === vouchers.length && vouchers.length > 0) {
  ...
  // Gunakan list 'vouchers' asli karena tidak ada filter/sortir
  const displayedVouchers = vouchers; 

  return (
  <div className="bg-white max-w-screen-2xl mx-auto h-full p-4 flex flex-col">
          ...
          {displayedVouchers.length > 0 ? (
            displayedVouchers.map((voucher) => (
              ...
              >
                ...
                <td className="px-6 py-4">
                  <span
                    className={`px-2 py-1 text-xs rounded-full ${getVoucherStatus(voucher).colorClass}`}
                  >
